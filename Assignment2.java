...
public class Assignment2 {
  ...
  public boolean chgRecord(int pid, int year, int wins, int losses) {
    try {
        ...
        ps = connection.prepareStatement("SELECT * FROM record WHERE pid = ? and year = ?");
        ps.setInt(1, pid);
        ps.setInt(2, year);
        rs = ps.executeQuery();
        if (!rs.next()){
      ...
      // Create the query
      ps = connection
          .prepareStatement("UPDATE record SET wins = ?, losses = ? WHERE pid = ? and year = ?");
  ...
  public int findTriCircle() {
    ...
    try {
      ps = connection.prepareStatement(
          "SELECT COUNT(*) FROM (SELECT DISTINCT e1.winid, e2.winid, e3.winid FROM event e1, event e2, event e3 WHERE e1.lossid = e2.winid and e2.lossid = e3.winid and e3.lossid = e1.winid) AS results");
      // Execute the query
      rs = ps.executeQuery();
      while (rs.next()) {
